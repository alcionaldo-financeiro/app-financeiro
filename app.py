...
if not st.session_state.autenticado:
    # Se já tem na URL, tenta logar direto ou preencher
    ...
    if st.button("ENTRAR ✅", type="primary"):
        ...
        if n_in and len(c_l) == 11:
            ...
            st.rerun()
        else: st.error("⚠️ CPF inválido.")
    st.stop()

# --- 4. APP ---
df_total = carregar_dados()
# Filtro: Apenas dados do usuário logado, que não foram excluídos e que não são datas futuras
...
with tab1:
    ...
    data_lanc = st.date_input("Data do Trabalho (Padrão: Hoje):", value=HOJE_BR, format="DD/MM/YYYY")
    ...
    v4 = col2.number_input("Particular", min_value=0.0)
    ...
    if st.button("SALVAR AGORA ✅", type="primary"):
        ...
        nova.update({'ID_Unico': str(int(time.time())), 'Status': 'Ativo', 'Usuario': st.session_state.usuario, 'CPF': st.session_state.cpf_usuario, 'Data': data_lanc.strftime("%Y-%m-%d"), 'Urbano': v1, 'Boraali': v2, 'app163': v3, 'Outros_Receita': v4, 'Energia': c1, 'Alimentacao': c2, 'KM_Inicial': k_ini, 'KM_Final': k_fim if k_fim > 0 else k_ini})
...
with tab2:
    if df_user.empty: st.info("Sem dados até hoje.")
        ...
        # 1. INDICADORES (KPIs)
        tr, tc, tl, tk = df_bi['Rec'].sum(), df_bi['Cus'].sum(), df_bi['Lucro'].sum(), df_bi['KMR'].sum()
        ...
        # 2. TABELA DE DADOS (COM COLUNA USUARIO)
        st.markdown("### 📋 Lançamentos até Hoje")
        df_exibir = df_bi.copy()
        df_exibir['Data'] = df_exibir['Data'].dt.strftime('%d/%m/%Y')
        cols_view = ['Data', 'Usuario', 'Rec', 'Cus', 'Lucro', 'KMR', 'KM_Inicial', 'KM_Final', 'ID_Unico']
        st.dataframe(df_exibir[cols_view], use_container_width=True, height=250)

        # 3. EXCLUSÃO
        with st.expander("🗑️ Excluir lançamento"):
            item_excluir = st.selectbox("ID", df_bi['ID_Unico'].tolist())
        ...
        # Barras: Receita vs Lucro
        fig1 = px.bar(df_g, x='Dia', y=['Rec', 'Lucro'], barmode='group', color_discrete_map={'Rec':'#28a745','Lucro':'#007bff'}, title="Evolução Diária")
        st.plotly_chart(fig1, use_container_width=True)
        
        # Pizza: Custos
        custos_data = {'Energia': df_bi['Energia'].sum(), 'Alimentação': df_bi['Alimentacao'].sum(), 'Outros': df_bi['Outros_Custos'].sum()}
        fig2 = px.pie(names=list(custos_data.keys()), values=list(custos_data.values()), hole=0.4, title="Distribuição de Custos")
